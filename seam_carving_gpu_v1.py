...
GRAY_IMG_SHAPE=BLOCK_SIZE_ENERGY+2
M_SHAPE=BLOCK_SIZE_ENERGY+2
@cuda.jit
def forward_energy_kernel_v1(gray_img, energy, m, r):
    s_gray_img = cuda.shared.array(shape=(GRAY_IMG_SHAPE, 2), dtype=float64)
    s_m = cuda.shared.array(shape=M_SHAPE, dtype=float64)
    ...
    if c < width:
        ...
        if tx == 0:
            s_gray_img[0, 0] = gray_img[r-1, left]
            s_gray_img[1, 0] = gray_img[r, left]
            s_m[0] = m[up, left]
        if tx == BLOCK_SIZE_ENERGY-1:
            s_gray_img[0, BLOCK_SIZE_ENERGY+1] = gray_img[r-1, right]
            s_gray_img[1, BLOCK_SIZE_ENERGY+1] = gray_img[r, right]
            s_m[BLOCK_SIZE_ENERGY+1] = m[up, right]
        
        s_gray_img[0, tx+1] = gray_img[r-1, c]
        s_gray_img[1, tx+1] = gray_img[r, c]
        s_m[tx+1] = m[up, c]
...
@cuda.jit
def get_minimum_cost_table_kernel_v1(energy, backtrack, w, r):
    s_energy = cuda.shared.array(shape=SMEM_MIN_COST_SHAPE, dtype=float64)
    c = cuda.grid(1)
    tx = cuda.threadIdx.x
    bx = cuda.blockIdx.x
    if (c < w):
...
def remove_seams_parallel(img, num_remove, checksum=False):
    ...
    for _ in range(num_remove):
        ...
        block_size_energy = BLOCK_SIZE_ENERGY
        grid_size_energy = math.ceil(width / block_size_energy)

        for r in range(1, height):
            forward_energy_kernel_v1[grid_size_energy, block_size_energy](
        ...
        ### get minimum seam
        seam_idxs = get_minimum_seam(min_costs, backtrack)
...
if __name__ == '__main__':
    ...
    print("SEAM CARVING GPU - OPTIMIZE 1")
